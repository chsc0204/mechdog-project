...
# 원본: Hiwonder 공식 04 手机APP遥控程序/main.py
        ...
        if (_COMMAND==3) and (_obstacle_avoidance_flag==0) and (_self_balancing_flag==0):
          ...
          if _RUN_DIR < 6:
            ...
            if dir_flag != -1:
              dir_flag = -1
              mechdog.transform([-10 , 0 , 0] , [0 , 0 , 0] , 100)
        # [추가] CMD|9|{deg}|$ : 순수 제자리 회전 (전진 없이 회전만)
        # deg 양수=좌회전, 음수=우회전. 예: "CMD|9|30|$" -> 왼쪽으로 30도 제자리 회전
      ...
      if (step==3):
        while True:
          if (_RUN_DIR==0):
            ...
            mechdog.move(-40,20)
            continue
      # [추가] step==9 : 순수 제자리 회전 (한 번 실행하고 자동 정지)
      if (step==9):
        mechdog.move(0, _ROTATE_DEG)
        time.sleep(1.0)   # 회전 동작 시간 확보 (필요시 이 값을 조정하세요)
        mechdog.move(0, 0)
      step = 0
